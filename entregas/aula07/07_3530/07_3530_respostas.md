Para resolver o labirinto, foi usado um algoritmo de busca em profundidade com recursão. Primeiro, uma cópia do labirinto é feita para que o labirinto original não seja modificado. Depois, é feita uma lista de recursões e, se não for a primeira chamada, é retirada da lista o oposto da direção seguida anteriormente (para que o programa não considere voltar atrás uma alternativa válida). Então a função busca por direções válidas e toma a primeira que encontrar, chamando a função para a casa correspondente a tomar aquela direção. Ao chegar num beco sem saída, a função não faz nada. Ao chegar no queijo, ela retorna True e modifica a casa atual para apontar para a direção do queijo, e esse True é então propagado a cada chamada na pilha. Ao chegar na chamada final, a cópia com as modificações é retornada.
